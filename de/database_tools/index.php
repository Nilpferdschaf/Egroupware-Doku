...
<body id="top">
	...
	<p>
		The first table you will create will create will just be a simple list
		of departments, containing the columns <span class="column">dep_id</span>
		and <span class="column">dep_name</span>. You will use this column to
		replace the list of departments from the previous tutorial, which is
		still hard-coded at the moment. <br> Open up the eTemplate editor to
		begin and click on <span class="button">DB tools</span> in the
		sidebox. This opens the table editor.
	</p>
	...
	<p>
		...
		the first one, enter <span class="string">dep_id</span> as <span
			class="option">dep_id</span>. Set the <span class="option">Type</span>
		to <span class="option">int</span> and tick the <span class="option">NOT
			NULL</span>, <span class="option">Primary Key</span> and <span
			class="option">Unique</span> check boxes. Now click on the <span
	...
	<pre class="command">chown -R www-data:www-data $egw_installation/helloworld/setup</pre>

	<p>
		in a terminal. Once that is done, click <span class="button">Write
			tables</span>. Updating the database always requires a new version
		number and an app upgrade in the <span class="link">$your_domain/egroupware/setup</span>
		area. Input the new version number and confirm that you want to create
		the table (Your number might differ from the screenshot).
	</p>

	<img src="img/28-write_table.png"
		alt="Click on Write Table to confirm your changes" />
	<img class="small" src="img/29-confirm.png" alt="The table editor" />

	<p>
		If you now take a look at <span class="file">helloworld/setup/setup.inc.php</span>
		you wil notice a new line at the end:
	</p>
	...
	<p>
		...
		create the first table, whereas the installer does. After deinstalling
	...
	<p>
		...
		command prompt should open.<br> Now type in <span class="string">SHOW
			DATABASES;</span>. A list of databases should appear.
	</p>
...
class helloworld_so {
	...
	<p>The class also needs two functions. The first one is the
		constructor, which clones the database object, sets it up for usage
		within your app and then buffers the table. The second one is just a
		getter-function.</p>
...
function __construct()
{
	...
	{
		$this->departments [$record ['dep_name']] = $record ['dep_id'];
	}
}
...
function get_department_names()
{
	...
	<p>
		...
		within that table. There are also other parameter which are not used
...
$selection_options = array(
	'department' => $this->so->get_department_names()
);</pre>
...
$this->so->get_department_names () [$content ['department']] . ".\n";</pre>
...
Query OK, 1 row affected (0.13 sec)
	...
	<p>
		Add new columns like in the screenshot below. Afterwards, click on <span
			class="button">Write Tables</span>. Egroupware will again ask you how
		to update the version number and require confirmation.
	</p>
	...
	<p>
		Do not forget to also increase the version number in <span
			class="file">setup/setup.inc.php</span>. While you are there, take a
		look at the newly generated file <span class="file">setup/tables_update.inc.php</span>.
		...
		file, that is why you had to completely reinstall the application.<br>
		Now that it exists, you can just go to <span class="url">$your_domain/egroupware/setup</span>
		and upgrade like you normally would.
	</p>
	...
	<pre class="code">
function insert_employee($fname, $sname, $position, $department)
{
...
function increment_emp_id() {
	...
	<p>
		The <span class="insert">function</span> takes as parameters a string
		containing the table name and an array of <span class="var">$column =>
			$value</span> pairs. There are a few optional parameters,
	</p>
	...
	<p>
		...
		Egroupware can not make columns AUTO_INCREMENT. You could add this
		manually via
	</p>
	...
	<p>
		For now, you need to use this new function in <span class="class">helloworld_ui</span>.
		Update the <span class="function">index</span> function like this:
	</p>
	<pre class="code">
function index($content = null)
{
	...
	<p>This will write every newly entered employee into the database in
		addition to displaying the welcome message. You can now try it out and
		insert a few random people. When you think you are done, got to the
		mysql command line anf enter</p>
...
function get_employees()
{
	...
	<p>
		Those are a lot of parameters! In order, the are called: <span
...
function insert_employee($fname, $sname, $position, $department) {
	return $this->so->insert_employee($fname, $sname, $position, $department);
}</pre>
...
class helloworld_ui
{
	...
	<ul>
		...
		<li>Select box : change[department]</li>

		<li>Button "Modify" : modify</li>
	...
	<p>
		...
		<span class="string">position</span> and <span class="string">department</span>.
	...
	<p>
		When you do this, you will see that Egroupware automatically adds the
...
function update_employee(
		...
		$position_change, $department_change
	)
{
	return $this->db->update (
		...
		array(
			...
			'dep_id' => $this->departments [$department_change]
		),
		<span class="comment">WHERE (comparing columns to constants)</span>
		array(
			'fname' => $fname_select,
	...
	<p>
		These both work very similar to the <span class="function">insert</span>
	...
	<p>
		...
		object. Add the follwing functions to <span class="class">helloworld_bo</span>.
	</p>
...
function index($content = null)
{
	...
	$selection_options = array(
			'department' => $departments,
			'change' => array('department' => $departments)
	);
	...
	<p>
		...
		pressed. <br> If a button is determined to be pressed, the respective
		function call is made and if the query executed succesfully, a note is
		printed to the debugging area.<br> Note that the department-changed
		select box also needs to be populated with the department entries
		through the <span class="var">$selection_options</span>.
	</p>
	...
	<p>
		...
		where they just don not cut it. For these cases there is another
