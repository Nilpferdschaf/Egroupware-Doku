...
<body id="top">
	...
	<p>Hooks are a software design pattern used to create an easy way to
		...
		do all of this in this tutorial.</p>
...
$setup_info ['helloworld'] ['hooks'] = array(
	...
	<pre class="code">'version' => '0.20',</pre>
...
function sidebox_menu()
{
	...
	<p>
		...
		either have key =&gt; value pairs or arrays. The key =&gt; value pairs
		encode links, where value contains the hyperlink and key is the string
		shown in the sidebox.<br> The arrays can encode any HTML you want by
		using the <span class="string">text</span> key. The corresponding
		value will be copied directly into the pages' HTML code. The other
		keys that can be used are <span class="string">nolang</span>,
		specifying wether EGroupware should try translating the value of <span
			class="string">text</span>. <span class="string">link</span> can be a
		url the item should link to and <span class="string">icon</span> can
		point to an image that will be displayed next to the item.
	</p>
	...
	<p>
		...
		key =&gt; value pairs. It will then return a link to the url with all
		the parameters concatenated to the end.<br> As you can see, the second
		links points EGW to a function <span class="function">db_controls</span>
		inside <span class="class">helloworld_ui</span>. Go on and add this
		function. Do not forget to include the corresponding in <span
			class="var">$public_functions</span>.
	</p>
...
function db_controls($content = null)
{
	...
	<p>Now it is finally time to load up EGroupware and experience in
		action everything you have done. Clicking the links should take you
...
$setup_info['helloworld']['hooks'] = array(
	...
	<p>
		...
		you want to use any of these, you should not forget to add them to the
...
static function settings()
{
	...
	<p>
		Do not forget to increase the version number and to go to setup to
...
static function admin()
{
	...
	<p>ACL stands for Access Control List. Using this hook you can tell
		...
		return it expects an array of key => value pairs, where the key is an
...
static function categories() {
	...
	<p>Finally there is the delete account hook. It gets called whenever a
		...
		about the user and make sure you're not causing any database
		inconsistencies in the process.</p>

	<h3>Conclusion</h3>

	<p>It is now time to finally move on to the next part, where you will
		learn how to use images and style sheets to make everything a little
		prettier.</p>
