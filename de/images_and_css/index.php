...
<body id="top">
	...
	<p>
		The default folder where images are stored in EGroupware is <span
			class='file'>templates/default/images</span>. If you want special
		images for other skins, like pixelegg, those images go into <span
	...
	<p>
		One of the first thing you might want to do is getting rid of the
		default navbar icon. The only thing you need to do to achieve this is
		...
		just use <a href="img/39-hello_world.png">this one</a>.
	</p>
	...
	<pre class="command">apachectl -k restart</pre>

	<p>Once it updates, the sidebar and navbar with your app will look like
		this:</p>
	...
	<p>
		...
		about <a href="../database_tools">databases</a>. When you created the
		<span class="button">Delete</span> submit button and set its <span
			class="option">Name</span> option to <span class="string">delete</span>,
	...
	<p>
		To make one of them, just use the related keyword as the <span
	...
	<p>
		...
		pixels. You can download a small collection of them <a
	...
	<p>If you do this and hit apply, the button should appear in the
		...
		the images and they work just like regular buttons.</p>
	...
	<p>
		...
		and <span class="string">select[sname]</span>, that you created, and
		enter <span class="string">,entry-select</span> to give it the class <span
...
.entry-select {
	...
	<p>Now open the database control page of your app and see if it worked.
		The two entry select boxes should habe a pink background.</p>
	...
	<p>
		...
		<span class="string">$widgetname</span> will be assigned the id <span
			class="string">$appname-$templatename_$widgetname</span>. Remember
		that there is a text area on the database control page whose <span
			class="option">Name</span> is <span class="string">debug</span> and
		add the following CSS to <span class="file">app.css</span>.
	</p>

	<pre class="code">
#helloworld-db_controls_debug {
	background: black;
	color: lightgreen;
}</pre>

	<p>Save the file and refresh EGroupware. The debug area will now be
		black with light green text.</p>

	<img src="img/48-styled_debug.png"
		alt="The debug area is now styled with CSS.">
	...
	<p>
		...
		the right click somewhere onto the page and select <span
	...
	<p>
		...
		moment the development tools only target root frame of the EGroupware
		website. Before you can use the developer tools, you will have to
		target the iframe containing your app. To do this, click on the button
		<span class="string">Select an iframe as the currently targeted
			document.</span>, as shown in the screenshot below. The iframe you
		want to choose is the one that ends in <span class="string">menuaction=helloworld.xyz.zyx</span>.
	</p>
	...
	<p>
		Firefox will now present you with a list of CSS files thare are all
	...
	<p>
		...
		ones you added earlier. The delete button of your app still looks a
		bit out of place. That is, because EGroupware automatically adds a
		certain margin before every delete button to separate it from other ui
		elements. This is usually very convenient, but looks bad in the case
		of this specific app. You can remove the margin by typing the
		following CSS rule directly into the Style Editor.
	</p>
...
#helloworld-db_controls_delete {
	...
	<p>The delete button will magically move to the left a bit until it
		stops right next to the previous text, where it belongs.</p>

	<img src="img/53-delete_rectified.png"
		alt="The delete button at its proper position.">
...
#helloworld-db_controls_change\[department\] {
	margin-left: 20px;
}</pre>
	...
	<p>
		...
		copy and paste everything you edited onto your EGroupware server. When
		you are editing in the Style Editor, you are only editing a local copy
		of the CSS file. Your changes need to be made permanent manually or
