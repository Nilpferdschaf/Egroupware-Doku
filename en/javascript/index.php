...
<body id="top">
	...
	<p>
		...
		developer surveys for over three years now. It is only natural that
		EGroupware integrates well with JavaScript. You will learn how it
...
app.classes.helloworld = AppJS.extend({
	...
	<p>
		...
		when all eTemplate functionality is loaded and ready to go. The
		arguments to this function contain references to the eTemplate2 main
		object and the name of the template. All three functions should first
		call their parent functions.
	</p>

	<p>
		Saving the et2 main object reference to a library variable is often
		useful, because it contains some helpful functions for manipulating
		the template. This can be done very easily by just including a
		variable <span class="var">et2</span> and initializing it to <span
			class="var">null</span>. The variable will automatically be set to be
		a reference to the eTemplate object. Whenever you do this, you should
		not forget to clean up the reference in the <span class="function">destroy</span>
		function.
...
app.classes.helloworld = AppJS.extend({
	...
	<p>
		Before you start adding more JavaScript, you will need to go to the
		...
		This can be a simple <span class="string">alert('hi');</span> or
		something like <span class="string">app.helloworld.preview();</span>,
	...
	<p>
		Add this code to the text fields fname, sname and position and to the
		department select box dep_id. The <span class="function">preview</span>
		function will be triggered whenever the content of one of these ui
		elements changes and update a little preview text that shows the user
		what will be inserted into the database. To display the preview text,
		add a label above the submit button and set its <span class="option">Name</span>
		to <span class="string">preview-text</span>.
	</p>
	...
	<p>
		The following is an implementation of the <span class="function">preview</span>
		function. You can use it in <span class="file">app.js</span>.
	</p>
...
preview : function() {
	...
	<p>
		...
		selected entry of a select box. The selected entry will be returned as
		an index number. To get the actual label of the entry, you need to
		access the <span class="var">options</span> property of the select
		box. It contains, next to other information, the <span class="var">selection_options</span>
		of the widget. Once you have those, you just need to get the correct
		label for the value.
	</p>

	<p>Depending on what has already been input by the user, a string is
		generated that describes the information that will be written to the
		database. Lastly the content of the preview-text label will be set to
		this string.</p>

	<p>Save the template and open your app. You can now start inputting a
	...
	<p>
		...
		eTemplate 2 JavaScript library. These are all accessed through the <span
			class="var">et2</span> variable. You have already seen one of these
		functions, namely <span class="function">getWidgetById</span>.
	</p>
	...
	<p>
		...
		source code can be found at <span class="file">$egw_install/api/js/etemplate/et2_core_widget.js</span>.
		The augmentations can be found inside the <span class="file">$egw_install/api/js/etemplate/et2_widget_$widgettype.js</span>
		files.
	</p>
	...
	<p>
		Now that the password boxes exist, it is time to make them
...
et2_ready : function(et2, templ_name) {
	...
	<p>
		...
		and returns the DOM object that represents the widget in the browser.
		Adding the KeyListeners is now just a regular call to <span
			class="function">addEventListener</span>.
	</p>
	...
	<p>
		Now you need to implement the two EventListener functions <span
			class="function">pass1_key_evt</span> and <span class="function">pass2_key_evt</span>.
	</p>
...
pass2_key_evt : function(evt) {
	...
	<p>
		...
		press ctrl+shift+k. To give you an idea of what you are looking at, go
		to <a
		...
		places your app inside an iFrame on the page. You need to select this
	...
	<p>
		...
		your app through the console. They are all referenced inside the <span
			class="var">app.helloworld</span> global variable. Type this into the
		command line and press return. If you now click on the object that is
		being returned, you can see all its attributes on the right, including
		the <span class="var">pass1</span>, <span class="var">pass2</span> and
		<span class="var">et2</span> variables.
	</p>
	...
	<p>
		...
		source code in the corresponding <span class="file">api/js/etemplate/et2_*.js</span>
...
>submit.getDOMNode().style.transform="rotate(-30deg)"
	...
	<p>
		...
		Like before, you need to select the right iframe in the top right
	...
	<p>You should now have a good idea how the EGroupware JavaScript
		...
		find the missing information. The next and also last section will
		cover some other tools and functions that might help you in the
		future.</p>
	...
	<div class="download">
		...
		<p>
			You can download the result of this section <a
				href="../../downloads/07-javascript.zip">here</a>.
		</p>
	</div>
