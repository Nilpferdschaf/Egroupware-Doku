...
<body id="top">
	...
	<p>eTemplate is the custom templating language used by EGroupware. It
		is based on XML, but luckily you do not need to worry about that most
		of the time. EGroupware comes included with a graphical editor for
		eTemplates. Some features are missing from it, so you will have to
		dive into the code sometimes, but thanks to the XML structure, editing
	...
	<p>
		To start editing a new template, open the eTemplate editor and enter <span
			class="string">helloworld.index</span> into the name field. By naming
		it this way, you tell EGW that you want to create a template called <span
			class="name">index</span> for the app <span class="string">helloworld</span>.
		You will see this naming convention a lot in the future. For now, it
		is important for you to remember to begin all template names with <span
			class="string">$appname.</span>.<br> Enter some version number and
		press <span class="button">Save</span>. Just ignore the <span
			class="option">Template</span> and <span class="option">Lang</span>
		fields for now.
	</p>
	...
	<p>Now hover with your mouse in the upper left corner of the big white
		...
		the mouse pointer. If you can not find it, try pressing Ctrl-A to make
	...
	<p>
		...
		Click <span class="button">Apply</span> and watch you changes appear
		in the background. Use the selection boxes at the top to insert a
		second column into the grid, after the column we are in now.
	</p>

	<p>
		There are two new pink rectangles now, one after each label. Double
		click the top one. This time the <span class="option">Type</span>
		should be <span class="option">Text</span>. Click <span class="button">Apply</span>
		...
		That is what the <span class="option">Name</span> field is for. Insert
	...
	<p>
		...
		have not touched yet, the one after <span class="option">Department:</span>
		...
		and its <span class="option">Name</span> <span class="string">dep_id</span>.
	...
	<p>
		Now navigate to the grid widget by using the grid link after <span
			class="option">Path</span> again. At the top, instead of the options
		for inserting rows and columns into the grid, you now have options to
		insert widgets before and after the grid itself. Use these options to
		insert a widget before the grid. Set its <span class="option">Type</span>
		to <span class="option">HBox</span> and put a <span class="string">4</span>
		...
		text fields, such that the form looks like the picture below. Set the
		<span class="option">Name</span> options of the first and second text
		field to <span class="string">fname</span> and <span class="string">sname</span>,
		respectively.
	</p>
	...
	<pre class="command">chown -R www-data $egw_installation/helloworld/templates</pre>
	...
	<p>
		...
		only writes your changes into a buffer, while <span class="button">Export
			XML</span> actually writes the changes into a file.<br> If you take a
		look at <span class="file">$egw_installation/helloworld/templates/default/</span>,
		you should now find a new file there called <span class="file">index.xet</span>.
	...
	<p>
		With the template complete, you need to integrate it with your code
		somehow. You will now start writing actual PHP code. Before you begin,
		you should take a look at Egrouwares <a
	...
	<p>
		...
		in <span class="string">_ui</span>, all business objects in <span
			class="string">_bo</span> and all storage objects in <span
			class="string">_so</span>. Files containing a class should be named <span
...
class helloworld_ui {
	...
	<p>
		...
		name, but again, it is convention to do it like this. Let us take a
		closer look at the last two lines. First of all, we create a new
	...
	<p>
		In the final line, the <span class="function">exec</span> function is
		called. This function automatically generates the entire page, without
		any additional work required. It takes three arguments, the first is a
		callback string in the format <span class="string">$app.$class.$function</span>.
		...
		contains all the information needed to populate the template with
		content. There is nothing to populate at this point, so it will be
		empty. The last argument is an array used to populate things like
		selection boxes, content lists, or content trees. We use it to set
		some options for the department selection box.
	</p>

	<p>
		You should also note the import on the first line, <span class="var">EGroupware\Api\Etemplate</span>.
		It is required to use the classes that belong to eTemplate.
	</p>

	<p>
		If you go to EGroupware and open the app, you will see... Hello
		World!. That's because we still need to edit <span class="file">helloworld/index.php</span>
...
header('Location: ../index.php?menuaction=helloworld.helloworld_ui.index');</pre>
	...
	<p>
		This is basically the same you did when setting the callback method
		for the <span class="function">exec()</span> function. It tells
		EGroupware to create an instance of <span class="class">helloworld_ui</span>
		and call its <span class="function">index</span> function.
	</p>

	<p>
		Finally, add the following variable definition to <span class="file">class.helloworld_ui.inc.php</span>:
	</p>
...
public $public_functions = array(
	...
	<p>
		Reopen the app in EGroupware. You will see the form you created. It
		does not do anything though. Clicking on <span class="button">Submit</span>
...
function index($content = null) 
{
	...
	<p>
		...
		interestingly, if <span class="var">$content</span> is an array, the
		data from the form is parsed into a string and output to the debug
		area. Try it out now.
	</p>
	...
	<p>
		...
		is created every time you reload the page: When <span class="function">index</span>
		is called via the <span class="var">menuaction</span> parameter.
		Inside the constructor you can also do some other fancy things, like
		setting the page title of your app. The updated file looks like this:
	</p>
...
class helloworld_ui
{
	...
	<p>
		...
		the more basic features. There is still lots to be learned in the
		later sections, but for now we will have a look at EGroupware's
		database tools.</a>
	</p>
	...
	<div class="download">
		...
		<p>
			You can download the result of this section <a
				href="../../downloads/02-using_etemplate.zip">here</a>.
		</p>
	</div>
